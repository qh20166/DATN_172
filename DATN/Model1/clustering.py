...
from config import (
    ...
    SPEED_CLUSTER_LABELS,
    SPEED_CLUSTER_RULE_DEFINITION,
)

def export_cluster_profile_table(
    df: pd.DataFrame,
    labels: np.ndarray,
    output_dir: Path,
    ...
    profile_df = (
        ...
        work_df.groupby("cluster")

        .agg(

            sample_count=("cluster", "size"),

            **{col: (col, "mean") for col in usable_features},
    ...
    if speed_column in profile_df.columns:
        profile_df = profile_df.sort_values(speed_column, ascending=True).reset_index(drop=True)

    if len(profile_df) == len(SPEED_CLUSTER_LABELS):
        profile_df["semantic_label"] = SPEED_CLUSTER_LABELS
        profile_df["traffic_state"] = profile_df["semantic_label"].map(
            SPEED_CLUSTER_RULE_DEFINITION
        )
    else:
        profile_df["semantic_label"] = ""
        profile_df["traffic_state"] = ""
    ...
    csv_path = output_dir / "selected_kmeans_cluster_profile.csv"
    ...
    print("=" * 80)
    print(f"[INFO] Saved cluster profile to: {csv_path}")
    print("=" * 80 + "\n")
...
def build_cluster_semantic_mapping(
    df: pd.DataFrame,
    labels: np.ndarray,
    speed_column: str = "currentSpeed",
) -> Tuple[Dict[str, str], pd.DataFrame]:
    """
    Ánh xạ Cluster ID của thuật toán sang nhãn trạng thái giao thông A-F.

    Lưu ý: Cluster ID như 0, 1, 2... không có ý nghĩa thứ tự.
    Vì vậy cần tính currentSpeed trung bình của từng cluster, sắp xếp tăng dần,
    rồi gán A cho cụm có tốc độ thấp nhất và F cho cụm có tốc độ cao nhất.
    """
    work_df = df.copy()
    work_df["cluster"] = labels

    if speed_column not in work_df.columns:
        return {}, pd.DataFrame()

    work_df[speed_column] = pd.to_numeric(work_df[speed_column], errors="coerce")
    profile_df = (
        work_df.dropna(subset=[speed_column])
        .groupby("cluster")
        .agg(
            avg_currentSpeed=(speed_column, "mean"),
            min_currentSpeed=(speed_column, "min"),
            max_currentSpeed=(speed_column, "max"),
            sample_count=(speed_column, "size"),
        )
        .reset_index()
        .sort_values("avg_currentSpeed", ascending=True)
    ...
    # Chỉ ánh xạ A-F khi số cụm của mô hình đúng bằng số nhãn trạng thái cố định.
    # Các mô hình như DBSCAN có thể sinh rất nhiều cluster/noise nên không thể gán trực tiếp A-F.
    if len(profile_df) != len(SPEED_CLUSTER_LABELS):
        return {}, profile_df

    available_labels = SPEED_CLUSTER_LABELS
    profile_df["semantic_label"] = available_labels
    profile_df["semantic_definition"] = profile_df["semantic_label"].map(
        SPEED_CLUSTER_RULE_DEFINITION
    )

    mapping = {
        str(int(row["cluster"])): str(row["semantic_label"])
    ...
    return mapping, profile_df
from io_utils import save_json

def attach_semantic_cluster_labels(df, labels, speed_column="currentSpeed"):
    output_df = df.copy()
    output_df["cluster"] = labels
    output_df["cluster_kmeans"] = labels

    semantic_mapping, _ = build_cluster_semantic_mapping(
        output_df,
        labels,
    ...
    if semantic_mapping:
        output_df["semantic_cluster"] = (
            output_df["cluster"].astype(str).map(semantic_mapping)
        )
...
def _find_knee_index(values: np.ndarray) -> int | None:
    if values.size < 3:
        return None
    ...
    first = np.array([x[0], y[0]], dtype=float)
    last = np.array([x[-1], y[-1]], dtype=float)
    line = last - first
    ...
    distances = np.abs(np.cross(line, first - points)) / line_norm
    knee_index = int(np.argmax(distances))

    if knee_index <= 0 or knee_index >= values.size - 1:
        return None

    return knee_index


def select_dbscan_parameters_by_kdistance(
    scaled_array: np.ndarray,
    output_dir: Path,
    ...
    n_features = int(scaled_array.shape[1])
    selected_min_samples = int(min_samples or max(2, n_features + 1))

    n_samples = scaled_array.shape[0]
    if sample_size is not None and n_samples > sample_size:
        rng = np.random.RandomState(random_state)
        sample_indices = rng.choice(n_samples, size=sample_size, replace=False)
        X_param = scaled_array[sample_indices]
    else:
        X_param = scaled_array

    selected_min_samples = min(selected_min_samples, X_param.shape[0] - 1)

    neighbors = NearestNeighbors(n_neighbors=selected_min_samples, n_jobs=-1)
    neighbors.fit(X_param)
    distances, _ = neighbors.kneighbors(X_param)
    k_distances = np.sort(distances[:, -1])

    knee_index = _find_knee_index(k_distances)
    if knee_index is None:
        selected_eps = float(np.percentile(k_distances, 90))
        criterion = "90th percentile fallback of k-distance curve"
    else:
        selected_eps = float(k_distances[knee_index])
        criterion = "k-distance knee"
    ...
    pd.DataFrame([{
        ...
        "n_param_samples": int(X_param.shape[0]),
        "criterion": criterion,
    }]).to_csv(output_dir / "dbscan_parameter_selection.csv", index=False)

    plt.figure(figsize=(8, 5))
    plt.plot(np.arange(1, len(k_distances) + 1), k_distances)
    plt.axhline(selected_eps, linestyle="--", label=f"selected eps={selected_eps:.4f}")
    plt.title(f"DBSCAN k-distance graph (MinPts={selected_min_samples})")
    plt.xlabel("Sorted sample index")
    plt.ylabel(f"Distance to {selected_min_samples}-nearest neighbor")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / "figure_dbscan_k_distance.png", dpi=DEFAULT_FIGURE_DPI)
    plt.close()
    ...
    return {
        ...
        "n_total_samples": int(n_samples),
        "n_param_samples": int(X_param.shape[0]),
        "figure_path": str(output_dir / "figure_dbscan_k_distance.png"),
        "csv_path": str(output_dir / "dbscan_parameter_selection.csv"),
    }
...
def save_cluster_visualizations(
    ...
    plot_df = attach_semantic_cluster_labels(df, labels)

    cluster_counts = (
        plot_df["semantic_cluster"]
        .value_counts()
        .reindex(SPEED_CLUSTER_LABELS)
        .dropna()
    )
    plt.figure(figsize=(8, 5))
    cluster_counts.plot(kind="bar")
    plt.title("Phan bo so luong tung cum")
    plt.xlabel("Traffic state label")
    plt.ylabel("So luong")
    plt.tight_layout()
    ...
    if len(scatter_columns) >= 2:
        ...
        ordered_labels = [
            label for label in SPEED_CLUSTER_LABELS
            if label in set(sample_df["semantic_cluster"].dropna())
        ]

        for cluster_id in ordered_labels:
            cluster_subset = sample_df[sample_df["semantic_cluster"] == cluster_id]
            ...
            plt.scatter(
                ...
                s=8,
                alpha=0.6,
                label=f"{cluster_id}: {SPEED_CLUSTER_RULE_DEFINITION.get(cluster_id, '')}",
            )
...
def build_method_payload(
    ...
    semantic_mapping, semantic_profile_df = build_cluster_semantic_mapping(df, labels)
    return {
        "method": method_name,
        ...
        "semantic_profile": semantic_profile_df.to_dict(orient="records") if not semantic_profile_df.empty else [],
    }
...
def fit_final_dbscan(
    ...
    if scaled_array.shape[0] == 1:
        labels = np.zeros(1, dtype=int)
        dummy_model = DBSCAN(eps=0.5, min_samples=2)
        metrics = {
            "n_clusters": 1,
            "n_noise": 0,
            "inertia": 0.0,
            ...
            "parameter_selection": {
                "eps": 0.5,
                "min_samples": 2,
                "criterion": "fallback_single_sample",
                "n_features": int(scaled_array.shape[1]),
                "n_total_samples": 1,
                "n_param_samples": 1,
            },
    ...
    dbscan_parameters = select_dbscan_parameters_by_kdistance(
        scaled_array,
        output_dir,
        min_samples=max(2, scaled_array.shape[1] + 1),
        sample_size=None,
        random_state=random_state,
    )

    dbscan_model = DBSCAN(
        eps=float(dbscan_parameters["eps"]),
        min_samples=int(dbscan_parameters["min_samples"]),
        metric="euclidean",
        algorithm="ball_tree",
        n_jobs=-1,
    )
    labels = dbscan_model.fit_predict(scaled_array)
    ...
    if n_clusters == 0:
        ...
        metrics = {
            ...
            "eps": float(dbscan_parameters["eps"]),
            "min_samples": int(dbscan_parameters["min_samples"]),
            "parameter_selection": dbscan_parameters,
        }
        return dbscan_model, labels, metrics
...
def run_kmeans_clustering_pipeline(
    ...
    print("[CLUSTERING] Dang tim eps bang k-distance va fit DBSCAN")
    ...
    clustered_df = attach_semantic_cluster_labels(df, kmeans_labels)    
    ...
    cluster_profile_payload = export_cluster_profile_table(
        df=df,
        labels=kmeans_labels,
        output_dir=output_dir,
    )

    kmeans_summary["cluster_profile"] = cluster_profile_payload
    ...
    metrics_payload = {
        ...
        "figure_paths": figure_paths + visualization_paths + [
            dbscan_metrics["parameter_selection"]["figure_path"]
        ] if dbscan_metrics.get("parameter_selection", {}).get("figure_path") else figure_paths + visualization_paths,
        "clustered_csv_path": str(clustered_csv_path),
        "evaluation_csv_path": str(evaluation_csv_path),
        "methods_comparison": methods_summary,
        "cluster_profile": cluster_profile_payload,
    }
    ...
    print("[CLUSTERING] Hoan tat KMeans, MiniBatchKMeans, GaussianMixture, DBSCAN va AgglomerativeClustering")
